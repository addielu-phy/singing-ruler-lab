...
數值API採保守的fail-closed契約：非法identifier、非有限輸入、IEEE-754溢位，以及任何非零derived result被計算成零、subnormal或落在normal邊界的保守浮點安全帶時，都會拋出`RangeError`，而不回傳可能誤導的近似值。State API只接受caller自己的data properties，會在運算前一次複製已知欄位；accessor與Proxy讀取錯誤不會混入計算。零阻尼沒有有限衰減時間，因此`decayTimeS`回傳`null`而非`Infinity`。UI允許的完整參數範圍遠離此數值邊界。
