...
數值API採保守的fail-closed契約：非法identifier、非有限輸入、IEEE-754溢位，以及非零的力、位移、振幅或模態形狀被計算成零、subnormal或落在normal邊界的保守浮點安全帶時，都會拋出`RangeError`，而不回傳可能誤導的近似值。UI允許的完整參數範圍遠離此數值邊界。
